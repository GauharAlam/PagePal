...
-- Stripe Processed Events (for webhook idempotency)
...
DROP POLICY IF EXISTS "Users manage their saved summaries" ON public.saved_summaries;
...
DROP POLICY IF EXISTS "Users manage their plans" ON public.user_plans;
...
CREATE INDEX IF NOT EXISTS idx_chat_history_page_url ON public.chat_history(user_id, page_url);
...
CREATE INDEX IF NOT EXISTS idx_saved_summaries_page_url ON public.saved_summaries(user_id, page_url);
...
-- Includes authorization check so users can only increment their own counter
-- ================================================================
CREATE OR REPLACE FUNCTION public.increment_usage(p_user_id uuid, p_field text)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  -- Authorize: calling user must match target user, or call must come from service role (auth.uid() is null)
  IF auth.uid() IS NOT NULL AND auth.uid() != p_user_id THEN
    RAISE EXCEPTION 'Unauthorized: cannot increment usage for other users';
  END IF;

  IF p_field = 'daily_summaries' THEN
    UPDATE public.user_plans SET daily_summaries = daily_summaries + 1, updated_at = now() WHERE user_id = p_user_id;
  ELSIF p_field = 'daily_chats' THEN
    UPDATE public.user_plans SET daily_chats = daily_chats + 1, updated_at = now() WHERE user_id = p_user_id;
  END IF;
END;
$$;

-- ================================================================
-- BYOK — User API Keys (encrypted at rest in application layer)
